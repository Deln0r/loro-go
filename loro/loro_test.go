...
func TestMergeStateMatchesToJSON(t *testing.T) {
	...
	for _, name := range []string{
		...
		"conc_text", "conc_map", "conc_list", "conc_text2", "conc_list2", "richtext", "mlist", "tree_simple", "tree_meta", "text_del", "list_del", "map_del", "two_changes", "cross_del", "rt_one", "rt_two", "rt_overlap", "counter", "unicode_text", "map_mixed", "text_cjk_del",
	} {
		blob, err := os.ReadFile(filepath.Join(dir, name+".update.bin"))
		if err != nil {
			t.Skipf("fixture %s missing: %v", name, err)
